...
func TestUsePaymaster(t *testing.T) {
	...
	mint, err := tokenContract.Mint(opts, wallet.Address(), AirdropAmount)
	...
	tokenBalanceBefore, err := tokenContract.BalanceOf(nil, wallet.Address())
	...
	paymasterTokenBalanceBefore, err := tokenContract.BalanceOf(nil, paymasterAddress)
	assert.NoError(t, err, "BalanceOf should not return an error")
	...
	tokenBalanceAfter, err := tokenContract.BalanceOf(nil, wallet.Address())
	assert.NoError(t, err, "BalanceOf should not return an error")

	paymasterBalanceAfter, err := client.BalanceAt(context.Background(), paymasterAddress, nil)
	assert.NoError(t, err, "BalanceAt should not return an error")

	paymasterTokenBalanceAfter, err := tokenContract.BalanceOf(nil, paymasterAddress)
	assert.NoError(t, err, "BalanceOf should not return an error")
